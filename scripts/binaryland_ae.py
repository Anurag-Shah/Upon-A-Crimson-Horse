...
if __name__ == "__main__":
		cond = 'check_variable = { which = uach_effect_ae value = %s }'
		body = 'add_aggressive_expansion = { who = ROOT value = %s apply_calc = no }'
		form = 'if = {\n\t\tlimit = {\n\t\t\t%s\n\t\t}\n\t\t%s\n\t}\n\telse = {\n\t\t%s\n\t}' % (cond, '%s', '%s')

		with open('output.txt', 'w') as f:
			f.write('province_effect_var = { #var #effect\n\t'+btree([i/10 for i in range(0, 10000)], form, body)+'\n}')
